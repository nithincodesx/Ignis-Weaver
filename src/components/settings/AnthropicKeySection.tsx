...
import { Sparkles, Copy, RefreshCw, Eye, EyeOff } from "lucide-react";

interface AnthropicKeySectionProps {
  apiKey: string;
  model: string;
...
export default function AnthropicKeySection({
  apiKey,
  model,
  onApiKeyChange,
  onModelChange,
    ...
    <div className="guild-card p-5 space-y-4">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b pb-3" style={{ borderColor: "var(--border-dim)" }}>
        <div className="flex items-center gap-2">
          ...
          <div>
            <h3 className="font-bold text-sm" style={{ color: "var(--text)" }}>Anthropic API Credentials</h3>
            <p className="mono-label text-[10px]">Powers Coding & Complex Analysis Agents</p>
          </div>
        </div>
        {connectionStatus && (
          <span
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="md:col-span-2 space-y-1.5">
          <label className="mono-label block text-[10px]">ANTHROPIC API KEY (SK-ANT-...)</label>
          <div className="flex items-center gap-2">
        ...
        <div className="space-y-1.5">
          ...
          >
            <option value="claude-3-5-sonnet-20241022">Claude 3.5 Sonnet (v2 Recommended)</option>
